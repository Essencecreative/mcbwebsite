...
export default function TarifGuide() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                setLoading(true)
                const data = await getInvestorCategories('tariff-guide', currentPage, 9)
                setItems(data.items || [])
                setTotalPages(data.totalPages || 1)
            } catch (error) {
                console.error("Error fetching tariff guide items:", error)
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <>
            <Layout backgroundImage="/assets/images/backgrounds/Investor-Banner.png" headerStyle={1} footerStyle={1} breadcrumbTitle="Tarif Guide" breadcrumbSubTitle="Tarif Guide" breadcrumbLink="/tarif-guide">
                <div>
                    {/*Blog Page Start*/}
