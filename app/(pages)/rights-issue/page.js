...
function RightsIssueContent() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                setLoading(true)
                // Use 'rights-issue' as the category
                const data = await getInvestorCategories('rights-issue', currentPage, 9)
                setItems(data.items || [])
                setTotalPages(data.totalPages || 1)
            } catch (error) {
                console.error("Error fetching rights issue:", error)
            } finally {
                setLoading(false)
            }
        }
    ...
    const displayTitle = "Rights Issue"

    return (
        <Layout backgroundImage="/assets/images/backgrounds/Investor-Banner.png" headerStyle={1} footerStyle={1} breadcrumbTitle={displayTitle} breadcrumbSubTitle={displayTitle} breadcrumbLink="/rights-issue">
            <div>
