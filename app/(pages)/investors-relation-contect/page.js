...
export default function InvestorsRelationContact() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                setLoading(true)
                const data = await getInvestorCategories('contact', currentPage, 9)
                setItems(data.items || [])
                setTotalPages(data.totalPages || 1)
            } catch (error) {
                console.error("Error fetching investor contact items:", error)
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <>
            <Layout backgroundImage="/assets/images/backgrounds/Investor-Banner.png" headerStyle={1} footerStyle={1} breadcrumbTitle="Investors Relation Contact" breadcrumbSubTitle="Investors Relation Contact" breadcrumbLink="/investors-relation-contect">
                <div>
                    {/*Blog Page Start*/}
