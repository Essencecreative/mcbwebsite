...
export default function AnnualGeneralMeeting() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                setLoading(true)
                const data = await getInvestorCategories('agm', currentPage, 9)
                setItems(data.items || [])
                setTotalPages(data.totalPages || 1)
            } catch (error) {
                console.error("Error fetching AGM items:", error)
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <>
            <Layout backgroundImage="/assets/images/backgrounds/Investor-Banner.png" headerStyle={1} footerStyle={1} breadcrumbTitle="Annual General Meeting" breadcrumbSubTitle="Annual General Meeting" breadcrumbLink="/annual-general-meeting">
                <div>
                    {/*Blog Page Start*/}
