...
export default function ShareholdingStructure() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                setLoading(true)
                const data = await getInvestorCategories('shareholding', currentPage, 9)
                setItems(data.items || [])
                setTotalPages(data.totalPages || 1)
            } catch (error) {
                console.error("Error fetching shareholding structure items:", error)
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <>
            <Layout backgroundImage="/assets/images/backgrounds/Investor-Banner.png" headerStyle={1} footerStyle={1} breadcrumbTitle="Shareholding Structure" breadcrumbSubTitle="Shareholding Structure" breadcrumbLink="/shareholding-structure">
                <div>
                    {/*Blog Page Start*/}
